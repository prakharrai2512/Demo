...
int test() {
    ...
    try {
        ...
        module1 = torch::jit::load("C:\\Users\\acer\\Desktop\\SA lab\\Demo\\data\\torchik.pt");
    ...
    try {
        // Deserialize the ScriptModule from a file using torch::jit::load().
        module2 = torch::jit::load("C:\\Users\\acer\\Desktop\\SA lab\\Demo\\data\\combusken.pt");
    ...
    torch::jit::Module data = torch::jit::load("C:\\Users\\acer\\Desktop\\SA lab\\Demo\\data\\data.pth");
    //data.to(device_type);
    ...
    auto det_out = module2.forward({ img }).toTuple()->elements()[0].toTensor();

    auto maskout = module1.forward({ img }).toTensorVector();
    //std::cout << maskout;
    auto ll_out = maskout[1];
    auto da_out = maskout[0];
